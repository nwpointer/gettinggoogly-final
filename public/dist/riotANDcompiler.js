...
;(function (window, undefined) {
  ...
  ;(function (riot) {
    ...
    prot.m = function (first, second) {
      if (first[0] && (!second || second[0])) go(first, second);else if (second) this.r(first, second);else this.r('@', first);
    };
  ...
  * @param   { Object } options - set the propery overriding the default options
  ...
  var parsers = (function () {
    ...
    var _html = {
      jade: function jade(html) {
        return _req('jade').render(html, { pretty: true, doctype: 'html' });
      }
    };

    var _css = {
      stylus: function stylus(tag, css) {
    ...
    var _js = {
      ...
      livescript: function livescript(js) {
        return _req('livescript').compile(js, { bare: true, header: false });
      },
      typescript: function typescript(js) {
  ...
  var compile = (function () {
    ...
    var BOOL_ATTRS = _regEx('^(?:disabled|checked|readonly|required|allowfullscreen|auto(?:focus|play)|' + 'compact|controls|default|formnovalidate|hidden|inert|ismap|itemscope|loop|' + 'multiple|muted|no(?:resize|shade|validate|wrap)?|open|reversed|seamless|' + 'selected|sortable|truespeed|typemustmatch)$'),
    ...
    function mktag(name, html, css, attrs, js, pcex) {
      ...
      return 'riot.tag2(' + q(name) + c + q(html) + c + q(css) + c + q(attrs) + ', function(opts) {\n' + js + s;
    }
    ...
    function parseAttrs(str) {
      ...
      while (match = HTML_ATTR.exec(str)) {
        ...
        if (!v) {
          ...
          if (v[0] !== DQ) v = DQ + (v[0] === "'" ? v.slice(1, -1) : v) + DQ;
    ...
    function splitHtml(html, opts, pcex) {
      ...
      if (html && _bp[4].test(html)) {
        var jsfn = opts.expr && (opts.parser || opts.type) ? compileJS : 0,
    ...
    function riotjs(js) {
      ...
      while (match = js.match(JS_ES6SIGN)) {
        ...
        if (toes5) match[0] = match[1] + 'this.' + match[2] + ' = function' + match[3];
      ...
      function skipBlock(str) {
        ...
        while (level && (match = re.exec(str))) {
          if (match[1]) match[1] === '{' ? ++level : --level;
        }
    ...
    function compileJS(js, opts, type) {
      ...
      if (!parser) throw new Error('JS parser not found: "' + type + '"');
    ...
    function compile(src, opts, url) {
      ...
      return label + src.replace(/\r\n?/g, '\n').replace(CUST_TAG, function (_, tagName, attribs, body, body2) {
        ...
        if (attribs) attribs = restoreExpr(parseAttrs(splitHtml(attribs, opts, pcex)), pcex);

        if (body2) body = body2;

        if (body && (body = body.replace(HTML_COMMENT, '')) && /\S/.test(body)) {

          if (body2) html = compileHTML(body2, opts, pcex, 1);else {

            body = body.replace(STYLE, function (_, _attrs, _style) {
              var scoped = _attrs && /\sscoped(\s|=|$)/i.test(_attrs);
              styles += (styles ? ' ' : '') + compileCSS(_style, tagName, getType(_attrs), scoped);
              return '';
            });
            ...
            body = blocks[0];
            if (body) html = compileHTML(body, opts, pcex, 1);

            body = blocks[1];
            if (/\S/.test(body)) jscode += (jscode ? '\n' : '') + compileJS(body, opts);
          }
        }
  ...
  */
  riot.compile = (function () {
    ...
    function GET(url, fn, opts) {
      ...
      req.onreadystatechange = function () {
        if (req.readyState === 4 && (req.status === 200 || !req.status && req.responseText.length)) fn(req.responseText, opts, url);
      };
  ...
  /* istanbul ignore next */
  if ((typeof exports === 'undefined' ? 'undefined' : _typeof(exports)) === T_OBJECT) module.exports = riot;else if (typeof define === 'function' && define.amd) define(function () {
